...
//            按钮.sizeToFit()
            按钮.setTranslatesAutoresizingMaskIntoConstraints(true)
            ...
            设置坐标(按钮, X坐标: 按钮X坐标, Y坐标: 0.0, 宽度: 每个按钮宽度, 高度: 表格视图Y坐标)
        }
    ...
    func 设置坐标(要调整的视图:AnyObject, X坐标 x:CGFloat, Y坐标 y:CGFloat, 宽度 w:CGFloat, 高度 h:CGFloat)
    {
        var 按钮横向对齐方式 = NSLayoutConstraint(item: 要调整的视图, attribute: NSLayoutAttribute.Left, relatedBy: NSLayoutRelation.Equal, toItem: self.view, attribute: NSLayoutAttribute.Left, multiplier: 1.0, constant: x)
        var 按钮纵向对齐方式 = NSLayoutConstraint(item: 要调整的视图, attribute: NSLayoutAttribute.Top, relatedBy: NSLayoutRelation.Equal, toItem: self.view, attribute: NSLayoutAttribute.Top, multiplier: 1.0, constant: y)
        var 按钮宽度 = NSLayoutConstraint(item: 要调整的视图, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: self.view, attribute: NSLayoutAttribute.Width, multiplier: 1.0, constant: w)
        var 按钮高度 = NSLayoutConstraint(item: 要调整的视图, attribute: NSLayoutAttribute.Height, relatedBy: NSLayoutRelation.Equal, toItem: self.view, attribute: NSLayoutAttribute.Height, multiplier: 1.0, constant: h)
        self.view.addConstraints([按钮横向对齐方式, 按钮纵向对齐方式, 按钮宽度, 按钮高度])
    }
