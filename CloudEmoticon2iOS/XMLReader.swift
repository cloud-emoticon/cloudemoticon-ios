...
class XMLReader: NSObject {
    ...
    func 数据转换为XML(data:NSData, URL识别数组:NSArray) {
        ...
        let root:AnyObject? = XML解析类库.dictionaryWithData(data)
        if (root == nil) {
            ...
            let category:NSArray = 根字典.objectForKey("category") as! NSArray
