...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
            [_delegate allFiles:globalInfo.number_entry];
		}
	}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	if( ret!=UNZ_OK )
	{
		[self OutputErrorMessage:@"文件打开失败"];
	}
	...
	do{
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"解压缩发生错误"];
			success = NO;
			break;
		}
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"获取文件信息时发生错误"];
			success = NO;
        ...
        NSString *nowFileName = [[NSString alloc] initWithCString:(const char*)filename encoding:NSUTF8StringEncoding];
        [_delegate nowFile:nowFileName];
...
//		NSString * strPath = [NSString  stringWithCString:filename encoding:NSUTF8StringEncoding];
		...
		while( fp )
		{
			...
			{
				[self OutputErrorMessage:@"加密检查失败"];
                doBreak = YES;
				break;
			}
			else 
