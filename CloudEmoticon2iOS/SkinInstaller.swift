...
class SkinInstaller: NSObject, YashiNetworkKitDelegate {
    ...
    func 启动安装任务(_ 下载文件网址:String) {
        if (预先校验下载路径(下载文件网址 as NSString)) {
            ...
            self.代理?.显示安装提示框(true, 标题: lang.uage("主题安装失败") as NSString, 内容: lang.uage("下载路径不正确") as NSString, 按钮: lang.uage("取消") as NSString?)
        }
    }
    ...
    func YashiNetworkKit实时汇报进度(_ 已下载字节数:Int64, 总计字节数:Int64, 当前进度百分比:Int64) {
        ...
        显示安装提示框(true,标题: lang.uage("正在下载") as NSString,内容: 信息字符串,按钮: nil)
    }
    ...
    func YashiNetworkKit请求结果(_ 当前下载类:YashiNetworkKit, 返回的网址:URL?, 返回的数据:Data?, 返回的文件:String?, 返回的状态码:URLResponse?, 错误信息:NSError?) {
        ...
        if (错误信息 != nil) {
            ...
            显示安装提示框(true,标题: lang.uage("下载失败") as NSString,内容: lang.uage("请检查网络后重试") as NSString,按钮: lang.uage("取消") as NSString?)
        } else {
    ...
    func 预先校验下载路径(_ 下载文件路径:NSString) -> Bool {
        if (下载文件路径.length > 11) {
            let 后缀:NSString = 下载文件路径.substring(from: 下载文件路径.length-4) as NSString
            if (!后缀.isEqual(to: ".zip")) {
                return false
            }
            let 前缀:NSString = 下载文件路径.substring(to: 7) as NSString
            if (!前缀.isEqual(to: "http://") && !前缀.isEqual(to: "https:/")) {
                return false
            }
            return true
        }
    ...
    func 解压缩临时文件(_ 临时文件路径:String) {
        显示安装提示框(true,标题: lang.uage("正在安装") as NSString,内容: NSString(format: "%@: %ld",lang.uage("文件大小"),网络下载器!.下载文件总大小),按钮: nil)
        ...
        if (文件管理器.fileExists(atPath: 目标文件夹路径)) {
            do {
                try 文件管理器.removeItem(atPath: 目标文件夹路径)
            } catch _ {
            }
        }
        ...
        for _ in 0...1 { //如果失败则重试一次
            ...
            if (本次解压缩成功 && 错误信息 == nil) {
                ...
                case 0:
                    ...
                    INI字典.setObject(目标文件夹特征码, forKey: "md5" as NSCopying)
//                    NSLog("INI字典 = %@",INI字典)
                    break
                case 1:
                ...
                do {
                    try 文件管理器.removeItem(atPath: 临时文件路径)
                } catch _ {
                }
                if (INI读取状态码 > 0) {
                    do {
        ...
        if (解压缩错误信息 == nil) {
            ...
            显示安装提示框(true,标题: lang.uage("安装完毕") as NSString,内容: "",按钮: lang.uage("确定") as NSString?)
            NSLog("[皮肤安装器]安装成功。")
            代理!.主题安装完成()
        } else {
            显示安装提示框(true,标题: lang.uage("安装失败") as NSString,内容: 解压缩错误信息! as NSString,按钮: lang.uage("取消") as NSString?)
            NSLog("[皮肤安装器]安装失败。")
        }
    }
